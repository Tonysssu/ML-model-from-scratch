...
def logistic_grad_func(theta, x, y):
    return np.dot((logistic_val_func(theta, x) - y).T, np.c_[np.ones(x.shape[0]), x])/x.shape[0]
...
def sklearn_logistic_regression():
    ...
    # X-train: shape:(nsample, nfeatures)
    # Y_train: sahpe:(nsample,)
